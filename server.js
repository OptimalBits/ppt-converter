...
function copyLastFile(folder){
  var matcher =  /\d+\./;

  var files = fs.readdirSync(folder);

  // Remove files that do not match de matcher
  files = files.filter(function(file){
    return file.search(matcher) != -1;
  });

  // Sort files
  files.sort(function(a,b){
    return parseInt(a.match(matcher)[0],10) - parseInt(b.match(matcher)[0],10)
  });

  // Get las file number
  var lastFile = files[files.length-1];
  var lastNumber = parseInt(lastFile.match(matcher)[0]);

  // Create next file name
  var newFile = lastFile.replace(matcher, lastNumber+1+'.');

  // Copy last file
  var srcFile = folder + '/' + lastFile;
  var destFile = folder + '/' + newFile;
